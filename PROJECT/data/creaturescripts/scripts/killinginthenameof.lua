		...
		for _, id in ipairs(started) do
			if isInArray(tasks[id].creatures, getCreatureName(target):lower()) then
				...
				if getCreatureStorage(cid, KILLSSTORAGE_BASE + id) < tasks[id].killsRequired then
					...
					doCreatureSay(cid, ""..getCreatureStorage(cid, KILLSSTORAGE_BASE + id).."/".. tasks[id].killsRequired .."",19)
					doPlayerSendTextMessage(cid, 27, getCreatureStorage(cid, KILLSSTORAGE_BASE + id) .. "/" .. tasks[id].killsRequired .. " " .. tasks[id].raceName .. " already killed.")
				end
			end
		end
	end
