...
function onPrepareDeath(cid, deathList, lastHitKiller, mostDamageKiller)
	if getPlayerStorageValue(cid, 501) == 1 then
		...
		doPlayerSave(cid)
			...
			battlefield_preiacao(cid)
			doBroadcastMessage("[BattleField] ".. getGlobalStorageValue(415) .." Players do Blue team sobreviveram ao evento e obteram a recompensa de "..battlefield_coins.." Event Coins!", MESSAGE_EVENT_ADVANCE)
	...
	elseif getPlayerStorageValue(cid, 502) == 1 then
		...
		doPlayerSave(cid)
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "[BattleField] Você foi o último sobrevivente do time perdedor e também ganhou recompensa!")
			 battlefield_preiacao(cid)
			doBroadcastMessage("[BattleField] ".. getGlobalStorageValue(414).." Players do Yellow team sobreviveram ao evento e obteram a recompensa de "..battlefield_coins.." Event Coins!", MESSAGE_EVENT_ADVANCE)
			setGlobalStorageValue(414, getGlobalStorageValue(414)-1)
		doPlayerSave(cid)
			for _, cid in ipairs(getPlayersOnline()) do
