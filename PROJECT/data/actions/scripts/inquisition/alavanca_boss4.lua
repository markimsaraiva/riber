...
local tempo4 = 10*1000 -- tempo para ALAVANCA VOLTAR ESTADO NORMAL


if item.uid == 3134 and item.itemid == 1945 then
if getPlayerStorageValue (cid, 15891) == 1 then

if getGlobalStorageValue(sttorage) < 3 then

doTransformItem(item.uid,item.itemid+1)
doCreatureSay(cid,"Alavanca 4 ativada",19)
addEvent(doTransformItem, tempo4, item.uid,1945)

setGlobalStorageValue(sttorage , getGlobalStorageValue(sttorage) + 1 )
addEvent(setGlobalStorageValue, tempo4, sttorage ,  -1 )
