...
local exp = 1 -- O quanto que você quer que dobre sua experiencia, por exemplo 2 é 2x as rates do seu server.
local expfinal = 1 --Não mude, isso é para a experiencia voltar ao normal.
local textofinal1 = "You have the experience bonus + 100%. Say !diamondorb or !extraexp"
local seuitem = 7632 -- seu item que dará double exp
...
if item.itemid == seuitem and (getPlayerStorageValue(cid, limitedeuso) - os.time() <= 0) then
  local hora = 3600
       ...
       setPlayerStorageValue(cid, limitedeuso2, 0) --desabilita o bronze orb
       setPlayerStorageValue(cid, io_expbonus2 , 0) -- desabilita o bronze orb
  ...
  if vip.hasVip(cid) == true then
   ...
   doPlayerSetExperienceRate(cid,getexprate + exp)
    doSendMagicEffect(frompos,efeito2)
     setPlayerStorageValue(cid, limitedeuso, os.time() + 3600)
    doPlayerSendTextMessage(cid,22,texto)

      if vip.hasVip(cid) == true then
        doPlayerSendTextMessage(cid,22,"[VIP]: You have now [120%] more EXP.")
      else
        doPlayerSendTextMessage(cid,22,"[NO-VIP]: You have now [100%] more EXP.")
      end

-- addEvent(potion,pausa,cid)
   setPlayerStorageValue(cid, io_expbonus , 1)
