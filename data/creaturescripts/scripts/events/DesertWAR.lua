...
function onPrepareDeath(cid, deathList, lastHitKiller, mostDamageKiller)
	if getPlayerStorageValue(cid, 51) == 1 then
		...
		doPlayerSave(cid)
			...
			doBroadcastMessage("".. getGlobalStorageValue(5) .." Players do Black Assassins team sobreviveram ao evento e obteram a recompensa de 80 Event Coins ,10 Gold Ingots e 5 RibeirOT Coins!")
	...
	elseif getPlayerStorageValue(cid, 52) == 1 then
		...
		doPlayerSave(cid)
			...
			doPlayerAddItem(cid,6527,40)
			doPlayerAddItem(cid,12695,2)
			doBroadcastMessage("".. getGlobalStorageValue(4).." Players do Red Barbarians team sobreviveram ao evento e obteram a recompensa de 80 Event Coins, 10 Gold Ingots e 5 RibeirOT Coins!")
			setGlobalStorageValue(4, getGlobalStorageValue(4)-1)
		doPlayerSave(cid)
			for _, cid in ipairs(getPlayersOnline()) do
