...
local function rollDice(roll, cc_count, pc_count, table_left_pos, table_middle_pos, npc)
	...
	if ((total_g) <= 1000000 and (total_g) >= 5000) then
		...
		if (roll == 1 and random_rollval <= 3) then
			...
			addEvent(doCreatureSay, 500, npc, "You win "..total_g.." gold coins, Congratulations!", TALKTYPE_SAY, false, 0)
		...
		doCreatureSay(npc, "Put the money you wish to bet on the counter, the minimum wager is 5K and the maximum wager is 1KK.", TALKTYPE_SAY, false, 0)
	end
	return true
end
...
function creatureSayCallback(cid, type, msg)
	...
	local table_left_pos = {x = 120, y = 45, z = 7}
	local table_middle_pos = {x = 120, y = 46, z = 7}
